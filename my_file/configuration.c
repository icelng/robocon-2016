...
/*****************************摄像头OV7670***************************************/	
	//摄像头时钟XCLK   PA8:MCO_XCLK
	GPIO_Configuration(GPIO_Pin_8,GPIO_Mode_AF, GPIO_Speed_100MHz,GPIO_OType_PP,GPIO_PuPd_NOPULL,GPIOA); 
	GPIO_PinAFConfig(GPIOA, GPIO_PinSource8, GPIO_AF_MCO);
	//摄像头RESET PG15
	GPIO_Configuration(GPIO_Pin_15,GPIO_Mode_OUT, GPIO_Speed_50MHz,GPIO_OType_PP,GPIO_PuPd_NOPULL,GPIOG); 
	//摄像头PWDN PG9  
	GPIO_Configuration(GPIO_Pin_9,GPIO_Mode_OUT, GPIO_Speed_50MHz,GPIO_OType_PP,GPIO_PuPd_NOPULL,GPIOG); 
    ...
    //D5:PB6  D6:PE5  D7:PE6
    //VSYNC:PB7  HREF:PA4  PCLK:PA6
  ...
  GPIO_Configuration(GPIO_Pin_6|GPIO_Pin_7|GPIO_Pin_8|GPIO_Pin_9|GPIO_Pin_11,GPIO_Mode_AF, GPIO_Speed_100MHz,GPIO_OType_PP,GPIO_PuPd_UP,GPIOC);
  GPIO_Configuration(GPIO_Pin_6|GPIO_Pin_7,GPIO_Mode_AF, GPIO_Speed_100MHz,GPIO_OType_PP,GPIO_PuPd_UP,GPIOB);
  GPIO_Configuration(GPIO_Pin_5|GPIO_Pin_6,GPIO_Mode_AF, GPIO_Speed_100MHz,GPIO_OType_PP,GPIO_PuPd_UP,GPIOE);
  GPIO_Configuration(GPIO_Pin_4|GPIO_Pin_6,GPIO_Mode_AF, GPIO_Speed_100MHz,GPIO_OType_PP,GPIO_PuPd_UP,GPIOA);
/***********************************************************************************/
...
void TIM2_Configuration()        //1 s 定时
{
	/*TIM2*/
  RCC_APB1PeriphClockCmd(RCC_APB1Periph_TIM2, ENABLE);
  TIM_DeInit(TIM2);
...
void dcmi_config(){
    DCMI_InitTypeDef dcmi_init_stru;

    DCMI_DeInit();  //初始化DCMI,填入缺省值
    DCMI_StructInit(&dcmi_init_stru);

    //配置DCMI 用于摄像头
    dcmi_init_stru.DCMI_CaptureMode = DCMI_CaptureMode_Continuous; //DCMI_CaptureMode_SnapShot拍照模式DCMI_CaptureMode_Continuous为连续采集
    dcmi_init_stru.DCMI_CaptureRate = DCMI_CaptureRate_All_Frame;  //全帧模式(帧率与摄像头的相同)
    dcmi_init_stru.DCMI_ExtendedDataMode = DCMI_ExtendedDataMode_8b;//OV7670是8位模式
    dcmi_init_stru.DCMI_PCKPolarity = DCMI_PCKPolarity_Rising ; //PCLK上升沿有效(从OV7670时序图可得知)
    dcmi_init_stru.DCMI_HSPolarity = DCMI_HSPolarity_Low; //数据无效时HREF的电平，(从OV7670时序图可得知)
    dcmi_init_stru.DCMI_VSPolarity = DCMI_HSPolarity_Low;  //数据无效时VSYNC的电平，(从OV7670时序图可得知)
    dcmi_init_stru.DCMI_SynchroMode = DCMI_SynchroMode_Hardware; //设置同步模式为硬件同步模式(相比硬件同步模式，还有嵌入同步码同步模式)
    DCMI_Init(&dcmi_init_stru);

    DCMI_ITConfig(DCMI_IT_FRAME|DCMI_IT_ERR|DCMI_IT_LINE|DCMI_IT_OVF,ENABLE);  //使能帧传输完成的中断
...
void dma_config(){
    ...
    dma_init_stru.DMA_BufferSize = 40000;  //暂时不同其意思
    dma_init_stru.DMA_Channel = DMA_Channel_1;  //!!!!通道1不能单只写数字1，否则，经调试发现，与CR寄存器相或之后，EN位置1，导致下面的寄存器无法被配置
    ...
    dma_init_stru.DMA_Memory0BaseAddr = (uint32_t)(g_ov7670_image);//(g_ov7670_image) ((u32)(0x60000000|0x0C000000|0x00000080));  //设置存储数据的内存基地址
    dma_init_stru.DMA_MemoryBurst = DMA_MemoryBurst_Single;  //每次传输一个数据单元
    dma_init_stru.DMA_MemoryDataSize = DMA_MemoryDataSize_Word ;  //半字传输
    dma_init_stru.DMA_MemoryInc = DMA_MemoryInc_Enable;  //内存每次接收到数据之后，地址都会自增
    dma_init_stru.DMA_Mode = DMA_Mode_Circular;  //DMA_Mode_Circular不停地传送
    dma_init_stru.DMA_PeripheralBaseAddr = (u32)(&(DCMI->DR));  //DCMI_DR_ADDR外设地址为DCMI的数据寄存器地址
    dma_init_stru.DMA_PeripheralBurst = DMA_PeripheralBurst_Single; //一次传一个数据单元
    dma_init_stru.DMA_PeripheralDataSize = DMA_PeripheralDataSize_Word; //外设数据单元大小为字
