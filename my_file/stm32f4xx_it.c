...
//		}
		if(EXTI_GetITStatus(EXTI_Line7) != RESET){
				...
				if(GPIO_ReadInputDataBit(GPIOF,GPIO_Pin_7) == 0){
						...
						param_switch(&g_control_param,1);  //切换参数组(过河流参数组)
...
void TIM1_UP_TIM10_IRQHandler (void){    //TIM1为30毫秒的中断
		static u32 pre_tim_cnt = 0;
	  if(TIM_GetITStatus(TIM1,TIM_IT_Update) != RESET ) 
  	{
			acar_update_alfa(g_gyro[0]);
			acar_while(0.01);
...
void TIM2_IRQHandler(void){
		static u32 tim2_cnt = 0;
	  if( TIM_GetITStatus(TIM2, TIM_IT_Update) != RESET ) 
  	{
			TIM_ClearITPendingBit(TIM2,TIM_FLAG_Update);//必须清除中断标志位否则一直中断
//			DCMI_CaptureCmd(ENABLE); 
//			myprintf(USART3,"FPS:%d\n",g_ov7670_cols_cnt);
		//	sprintf(str_temp,"tim2_cnt:%d",tim2_cnt++);
		//	lcd_show_string(10,100,200,100,16,str_temp);
	//		myprintf(USART1,"2v300\r");
			g_ov7670_cols_cnt = 0;
		}	
}
//蓝牙串口中断处理函数
				...
				if(c_now == '['){
						...
						switch(cmd){
							case 'A':
								...
								ov7670_reg_write(camera_reg_addr,camera_reg_data);
							...
							case 'F':
								...
								break;  //'F'break;
							case 'P':
					//			mpu6050_cycleread_stop();   //由于用蓝牙传送信息比较慢，在这段时间如果还开着DCMI，可能DMA传输使得数组越界,而且 DCMI接口与FLASH写有冲突，所以，写FLASH之前先关闭DCMI
								...
								if(strcmp((char *)str_param[1],"save") == 0){
									...
									//	ov2640_start();
										...
										return;
								}else if(strcmp((char *)str_param[1],"alfa_adj") == 0){
										mpu6050_gyro_adj(MPU6050_GYRO_X);
										if((p_list_node = list_search(&g_param_list,"gyro_x_adj")) != NULL){
												p_list_node->data->param_value[param_group] = mpu6050_get_gyro_adj(MPU6050_GYRO_X);
										}else{  //如果链表中不存在传过来的参数，则插入新的节点
												memset(param_value_array,0,sizeof(float)*PARAM_GROUP_LENGTH);
												param_value_array[param_group] = mpu6050_get_gyro_adj(MPU6050_GYRO_X);
												list_insert(&g_param_list,list_get_length(&g_param_list)+1,"gyro_x_adj",param_value_array);
										}
										param_update_all();
										return;
								}
